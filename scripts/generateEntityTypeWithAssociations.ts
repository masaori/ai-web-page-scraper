...
export const generateEntityTypeWithAssociations = async () => {
  ...
  for (const entityDefinition of entityDefinitions) {
    ...
    export class VectorDb${pascalCase(entityDefinition.name)}Repository extends VectorDbRepository<${pascalCase(entityDefinition.name)}> {
      constructor(qdrantClient: QdrantClient, openAiClient: OpenAiClient) {
        super(qdrantCollectionName, is${pascalCase(entityDefinition.name)}, qdrantClient, openAiClient)
      }
