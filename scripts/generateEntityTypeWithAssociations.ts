...
export const generateEntityTypeWithAssociations = async () => {
  ...
  for (const entityDefinition of entityDefinitions) {
    const uniqueReferredEntities = identifiableEntities.filter((candidateEntityDefinition) => {
    ...
    const nonUniqueReferredEntities = identifiableEntities.filter((candidateEntityDefinition) => {
      return candidateEntityDefinition.properties.find((property) => property.isReference && !property.isUnique && property.targetEntityDefinitionName === entityDefinition.name)
    })
    ...
    console.log(
      'uniqueReferredEntities',
      uniqueReferredEntities.map((entityDefinition) => entityDefinition.name),
    )
    console.log(
      'nonUniqueReferredEntities',
      nonUniqueReferredEntities.map((entityDefinition) => entityDefinition.name),
    )
    console.log(
      'subTypeEntities',
    ...
    ${[...uniqueReferredEntities, ...nonUniqueReferredEntities]
