...
export const generateFormatInstructions = async () => {
  ...
  const pathToOutputDirectory = path.join(__dirname, '..', 'src', 'adapter', '_shared', '_gen', 'formatInstructions')
  ...
  for (const entityDefinition of entityDefinitions) {
    ...
    export const ${camelCase(entityDefinition.name)}FormatInstruction = (${necessaryProperties.map((property) => `${camelCase(property.name)}: string`).join(', ')}): string => {
      return \`{
      ${[
        ...enumProperties.map((property) => `"${camelCase(property.name)}": ${getTypeName(property)}`),
        ...necessaryProperties.map((property) => `"${camelCase(property.name)}": ${getTypeName(property)}:\${${camelCase(property.name)}}`),
      ].join(',\n      ')}
    }\`
  }
  `
